...
/**
 * Nullability of a nonterminal means that it can be "nothing", i.e. there is a
 * rule that contains the nonterminal on the left side and no symbol on the
 * right side (empty right side), expressing an optional rule.
 * <p>
 * <b>Algorithm:</b><br>
 * Sort rules by size. Search every rule that contain the nonterminal on left
 * side. Scan the left side of every rule until a terminal or a non-nullable
 * nonterminal appears. If found, rule is not nullable, else it is nullable. If
 * one of the rules that derive nonterminal is nullable, this nonterminal is
 * nullable.
 * 
...
class Nullable extends Hashtable {
	...
	/**
	 * Explores the nullability of all nonterminals in syntax. Provides
	 * nullability as Boolean within this Map, key is nonterminal.
	 * 
	 * @param syntax
	 *            the grammar to scan for nonterminals and their nullability.
	 * @param nonterminals
	...
	private boolean checkNullability(Syntax syntax, String nonterm, Map done)
		...
		for (int j = 0; j < syntax.size(); j++) {
			...
			if (nt.equals(nonterm) && rule.rightSize() <= 0) // this rule
																// derives the
																// nonterminal
																// and is empty
		...
		for (int j = 0; j < syntax.size(); j++) {
			...
			if (nt.equals(nonterm)) { // this rule derives the nonterminal
				...
				for (int i = 0; nullable && i < rule.rightSize(); i++) {
					...
					if (Token.isTerminal(symbol)) { // a terminal ends
													// symbol-loop
						nullable = false;
					} else if (symbol.equals(nonterm) == false) { // do not
																	// search
																	// self
						try {
							nullable = checkNullability(syntax, symbol, done); // is
																				// nullable
																				// when
																				// this
																				// symbol
							...
							throw new ParserBuildException("Nullable ERROR: "
									+ ex.getMessage() + " <- " + nonterm);
						}
					}
				}

				if (nullable) // one nullable rule is enough for nonterminal to
								// be nullable
					return putSymbol(nonterm, true);
			}
		}
	...
	public static boolean isNull(String symbol) {
		return symbol.length() <= 0
				...
				|| symbol
						.equals("" + Token.COMMAND_QUOTE + Token.COMMAND_QUOTE)
