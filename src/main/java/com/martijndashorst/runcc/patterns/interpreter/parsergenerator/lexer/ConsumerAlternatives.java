...
import com.martijndashorst.runcc.util.Equals;

/**
 * Container for alternative rules/Consumers. If one of the contained consumers
 * succeeds, the alternative succeeds, else it fails.
 * 
...
class ConsumerAlternatives extends Consumer {
	...
	/**
	 * Tries to match to all alternates. Returns true when at least one
	 * succeeds.
	 */
	@Override
	...
	/**
	 * Returns the common start character if all alternatives have the same,
	 * else null.
	 */
	@Override
	public Character getStartCharacter() {
		...
		for (int i = 0; i < alternates.size(); i++) {
			...
			if (i == 0)
				c = c1;
			else if (Equals.equals(c, c1) == false)
				return null;
		}
	...
	/**
	 * Returns 1 if start character exists, else the sum of possible start
	 * variances of all consumers (silently assuming that they have different
	 * character sets).
	 */
	@Override
	...
	/**
	 * Returns the maximum fixed length of all alternating consumers. A fixed
	 * start sequence ends at the first found character set (like "0..9").
	 * <p>
	 * Returns the maximum start length of all alternating consumers. A consumer
	 * start length ends at the first found repeatable or nullable consumer
	 * (like "chars*").
	 */
	@Override
	...
	protected ResultTree consumeInternal(InputText input) throws IOException {
		if (strategy == null) { // sort alternatives by their start/fixed length
			strategy = strategyFactoryMethod != null ? strategyFactoryMethod
	...
	/**
	 * Returns true if the passed consumer could be concurrent with one of the
