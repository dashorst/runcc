package com.martijndashorst.runcc.patterns.interpreter.parsergenerator;
...
/**
 * The universal bottom-up parser algorithm. Runs with a Lexer (containing the
 * input), ParserTables (containing the syntax), and a Semantic (optional).
 * 
 * <pre>
...
public class Parser implements Serializable {
	...
	/**
	 * Create a generic bottom-up Parser with passed ParserTables (representing
	 * the current syntax to apply).
	 * 
	 * @param tables
	 *            ParserTables representing the syntax.
	 */
	 ...
	 * Retrievable after parsing.
	 */
	public Object getResult() {
		return result;
	}

	// /** Sets if SHIFT/REDUCE or REDUCE/REDUCE conflicts wil be shown on
	// System.err. */
	...
	/**
	 * Sets the lexer to be used for parsing. The Lexer contains (or will
	 * contain) the input to parse. The Parser calls <i>setTerminals()</i> on
	 * this call.
	 */
	public void setLexer(Lexer lexer) {
		boolean initLexer = (this.lexer != lexer); // look if passed lexer needs
													// terminals
		this.lexer = lexer;
		clear(); // clear if reused
		if (initLexer)
			lexer.setTerminals(getParserTables().getTerminals()); // pass
																	// terminals
																	// to lexer
	}

	/**
	 * Returns the lexer that was set to this parser, to call <i>setInput()</i>
	 * to the lexer.
	 */
	...
	/**
	 * Sets the input to contained lexer, or throws IllegalStateException if no
	 * lexer was set.
	 */
	public void setInput(Object input) throws IOException {
		if (lexer == null)
			throw new IllegalStateException(
					"Can not set input when no lexer was defined!");
		clear(); // clear if reused
	...
	/**
	 * Default is true. When true, the Parser will pass a Map of expected
	 * symbols to Lexer at every token request.
	 */
	...
	private void semanticPush(Object result, Token.Range range) {
		if (semantic != null) { // when a semantic is present
			...
			rangeStack.push(range); // and its start-end positions within input
									// text
		}
	}
	...
	private void semanticPop(int popIndex, int countOfPops) {
		if (semantic != null) {
			...
			if (popIndex == 0) // first pop of right side holds last token value
				this.range = new Token.Range(null, currentRange.end); // helper
																		// to
																		// remember
																		// end
																		// address

			if (popIndex == countOfPops - 1) // if it is the last pop, make a
												// valid range for next push()
				this.range = new Token.Range(currentRange.start, this.range.end);
		}
	}

	/**
	 * Reduce a rule when input satisfied it. Pop the stack n times, n is the
	 * number of right symbols of the rule. Semantic gets called with all input
	 * tokens corresponding to the rule, if not null. A new state gets pushed,
	 * determined by the new state (after pops) and the nonterminal of the rule
	 * (left side).
	 */
	...
	/**
	 * Push a new state upon state stack, determined by the GOTO table with
	 * current state and the received token symbol. Then read a new token from
	 * Lexer, trying to evaluate a rule.
	 */
	protected Token shift(Token token) throws IOException {
		if (DEBUG)
			dump("shift from token symbol >" + token.symbol + "<");

		push(getParserTables().getGotoState(top(), token.symbol), token.text,
				token.range);
		dumpStack();
	...
	/**
	 * Delivers the next token from lexer to parser. Override to convert the
	 * Token value.
	 */
	protected Token getNextToken() throws IOException {
		Map expected = passExpectedToLexer && top().intValue() >= 0 ? getParserTables()
				.getExpected(top()) : null;
	...
	/**
	 * Parse the tokens returned from passed lexer. This call is for checking
	 * correctness without semantics.
	 * 
	 * @param lexer
	...
	/**
	 * Parse the tokens returned from passed lexer. This call is for processing
	 * input with semantics. At least <i>setLexer()</i> must have been called
	 * before.
	 * 
	 * @param semantic
	...
	public boolean parse(Semantic semantic) throws IOException {
		if (lexer == null)
			throw new IllegalStateException(
					"No lexer was defined to scan input!");
		setSemantic(semantic);
		return parse();
	}

	/**
	 * Parse the tokens returned from passed input. At least <i>setLexer()</i>
	 ...
	 * @return true when input was syntactically correct.
	 */
	public boolean parse(Object input) throws IOException {
		setInput(input);
		return parse();
	}

	/**
	 * Parse the tokens returned from passed lexer. This call is for integrating
	 * a semantic.
	 * 
	 * @param lexer
	 *            Lexer containing the input to parse
	 * @param semantic
	 *            the semantic to apply to parser results.
	 * @return true when input was syntactically correct.
	 */
	 ...
	 * @return true when input was syntactically correct.
	 */
	public boolean parse(Object input, Semantic semantic) throws IOException {
		setInput(input);
		setSemantic(semantic);
		return parse();
	}
	...
	/**
	 * Start parsing after setting Lexer and optionally Semantic. At least
	 * <i>setLexer()</i> must have been called before.
	 * <p>
	 * Init the parser, read first token, push state 0 and set action to SHIFT.
	 * Loop while action is not ERROR or ACCEPT, and token symbol is not ERROR,
	 * and top of stack is not ERROR. Within loop, get next action from
	 * PARSE-ACTION table using current state and token symbol. When action
	 * greater than zero, call reduce(), else when action is SHIFT, call
	 * shift().
	 * 
	 * @return true when input was syntactically correct.
	 */
	public boolean parse() throws IOException {
		...
		if (DEBUG)
			dump("initial token symbol >" + token.symbol + "<, text >"
					+ token.text + "<");

		while (token.symbol != null && // lexer error
				action.equals(ParserTables.ACCEPT) == false && // input accepted
				action.equals(ParserTables.ERROR) == false && // parse-action
																// table error
	 ...
	 * 
	 * @param action
	 *            current action from PARSE-ACTION table.
	 * @param token
	 *            recently received Token.
	 * @return action to proceed with. Token.symbol may not be null and current
	 *         state may not be ERROR after this call.
	 */
	 ...
	 * 
	 * @return true when action is ACCEPT, token.symbol is EPSILON, and state is
	 *         not ERROR.
	 */
	protected boolean detectError(Token token, Integer state, Integer action) {
		...
		if (token.symbol == null || action.equals(ParserTables.ERROR)) {
			if (token.symbol == null)
				ensureOut().println(
						"ERROR: Unknown symbol: >" + token.text + "<, state "
								+ state);
			else
				ensureOut().println(
						"ERROR: Wrong symbol: "
								+ (Token.isEpsilon(token) ? "EOF"
										: token.symbol + ", text: >"
												+ token.text + "<")
								+ ", state " + state);
			...
			ret = false;
		} else if (state.equals(ParserTables.ERROR)) { // ERROR lies on stack,
														// from SHIFT
			pop(1);
			ensureOut().println(
					"ERROR: found no possible follow state for " + top()
							+ ", text >" + token.text + "<");
			lexer.dump(out);
			ret = false;
			...
			lexer.dump(out);
			ret = false;
		} else if (action.equals(ParserTables.ACCEPT) == false) {
			ensureOut().println(
					"ERROR: Could not achieve ACCEPT. Symbol: " + token.symbol);
			lexer.dump(out);
			ret = false;
		}
