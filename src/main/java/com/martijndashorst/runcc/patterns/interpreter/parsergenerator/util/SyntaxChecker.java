 ...
 * <li>singular rules (nonterminal can be substituted by its singular right
 * symbol)</li>
...
public class SyntaxChecker {
	...
	public SyntaxChecker(Object syntaxFile) throws Exception {
	...
	public SyntaxChecker(Syntax syntax) {
		...
		System.out
				.println("Number of rules (after resolving parenthesis, alternations and wildcards): "
						+ syntax.size());
		...
		} else if (topLevelRules.size() < 1) {
			System.out
					.println("WARNING: Found no toplevel rule, first rule (default START rule) is: "
							+ syntax.getRule(0));
		} else {
			System.out
					.println("Start rule is \"" + topLevelRules.get(0) + "\"");
		}

		// check for unresolved nonterminals
		Set unresolved = syntax.getUnresolvedNonterminals();
		if (unresolved.size() > 0) {
			System.out.println("Found " + unresolved.size()
					+ " unresolved nonterminals:");
			diagnosis = false;
		...
		for (int i = syntax.size() - 1; i >= 0; i--) {
			...
			for (int j = 0; found == false && j < syntax.size(); j++) {
				if (j != i) {
					...
					for (int k = 0; found == false && k < rule2.rightSize(); k++)
						if (rule2.getRightSymbol(k).equals(
								rule.getNonterminal()))
							found = true;
				}
			}
			...
			if (found == false)
				System.out
						.println("WARNING: Found isolated (unused, redundant) rule: "
								+ rule);
		}
		...
		for (int i = syntax.size() - 1; i >= 0; i--) {
			Rule rule = syntax.getRule(i);
			boolean singular = (rule.rightSize() == 1 && topLevelRules
					.contains(rule) == false); // has only one symbol on right
												// side

			// check if defined only once on any left side
			for (int j = 0; singular && j < syntax.size(); j++)
				if (j != i
						&& syntax.getRule(j).getNonterminal()
								.equals(rule.getNonterminal()))
					singular = false; // nonterm has been found once more on
										// left side

			if (singular) {
				System.out
						.println("INFO: Found singular rule (nonterminal could be substituted by its right symbol): "
								+ rule);
				singulars++;
			}
		}
	...
	/**
	 * Returns false when the checked syntax is not able to be used as lexer or
	...
	public static void main(String[] args) {
		if (args.length <= 0) {
			System.err.println("SYNTAX: java " + SyntaxChecker.class.getName()
					+ " file.syntax [file.syntax ...]");
			System.err
					.println("	Prints out a diagnosis of passed syntax file(s).");
			System.exit(2);
		}
