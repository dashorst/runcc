...
/**
 * Consuming characters (or bytes) means reading from an input stream as long as
 * the read characters match the pattern the consumer was built for. A consumer
 * can be built with a fixed string, a character set, other character consumers,
 * or a mixed sequence of those. It can be a list of alternating (parallel)
 * consumers. It can be set repeatable and nullable. A character consumer
 * succeeds when he could apply all consumers or patterns/sets stored in its
 * sequence when reading input. It fails and unreads all characters when one
 * non-nullable consumer in its sequence fails.
 * 
...
class Consumer implements Comparable, Serializable {
	...
	/**
	 * Empty consumer knowing its rule. This could become a Lexer toplevel
	 * consumer.
	 */
	Consumer(Rule rule) {
		this.rule = rule;
	...
	public void append(String charOrString) {
		Object o = sequence.size() > 0 ? sequence.get(sequence.size() - 1)
				: null; // check preceding
	...
	/**
	 * Add a character set by its high character. Low character is previously
	 * appended one.
	 */
	public void appendSet(String high) throws LexerException {
		String low = (String) sequence.get(sequence.size() - 1); // throws
																	// ClassCastException
																	// if not
																	// String
		if (low.length() > 1) { // low character was appended to previous string
			int i = low.length() - 1;
			sequence.set(sequence.size() - 1, low.substring(0, i)); // cut last
																	// character
			sequence.add(new CharacterSet("" + low.charAt(i), high)); // append
																		// set
		} else {
	...
	/**
	 * Implements Comparable: Sort alternatives by precedence: -
	...
	/**
	 * Returns the first character of this consumer, or null if starting with a
	 * set.
	 */
	...
	/**
	 * Returns the count of possible start characters. For fixed start character
	 * this is 1.
	 */
	public int getStartVariance() {
		...
		if (o instanceof Consumer)
			...
			throw new IllegalStateException(
					"No fixed start character, no character set, where am i?");
	...
	/**
	 * The fixed length ends before the first found character set (like "0..9")
	 * or repeatable or nullable consumer.
	 */
	...
	/**
	 * The start length ends before the first found repeatable or nullable
	 * consumer (like "chars*"), but not before a character set.
	 */
		...
		for (int i = 0; breakIndicator.size() <= 0 && i < sequence.size(); i++) {
			...
			if (o instanceof Consumer) {
				...
				if (cc.isNullable()) { // fixed start length ends here
					...
					return len;
				} else if (cc.isRepeatable()) { // fixed start length ends here
	...
	/**
	 * Return contained consumer when having only one and no constraints. This
	 * is called immediately after construction.
	 */
	Consumer optimize() {
		if (constraints == null && sequence.size() == 1
				&& sequence.get(0) instanceof Consumer) {
	...
	/**
	 * Returns true if the rule of this consumer matches the passed left
	 * recursive rule. E.g. passing "nonterm ::= nonterm something" would match
	 * "nonterm ::= something".
	 */
	...
	/**
	 * Passes the factory for Strategy objects to all contained
	 * ConsumerAlternatives.
	 */
	public void setStrategyFactoryMethod(
			StrategyFactoryMethod strategyFactoryMethod) {
	...
	/**
	 * Reads from input. Returns null if input did not match, else a result tree
	 * containing the text. Ensures that a nullable consumer never returns null
	 * and a repeatable consumer repeats.
	 * 
	 * @param input
	 *            Input object where to read from.
	 * @return null if no match, else scanned input as a result tree.
	 */
	public ResultTree consume(InputText input) throws IOException {
		// prepare scanning for results
		ResultTree result = isRepeatable() ? ensureResultTree(null) : null; // prepare
																			// a
																			// list
																			// container
																			// when
		...
		if (result == null && isNullable()) // having read no input should not
											// break loop of caller
			result = ensureResultTree(null); // return empty result element

		// add range when there was a result
		if (result != null) {
			Token.Address end = new Token.Address(input.getScanLine(),
					input.getScanColumn(), input.getScanOffset());
	 ...
	 */
	protected ResultTree consumeInternal(InputText input) throws IOException {
		...
		for (int i = 0; ok && i < sequence.size(); i++) {
			...
			for (int j = 0; ok && constraints != null && j < constraints.size(); j++) {
				...
				int mark1 = input.getMark(); // set mark as constraint-consumer
												// is no permitted consumer
				ok = (cc.consumeInternal(input) == null); // ok when constraint
															// failed to read
			...
			if (ok) { // no constraint succeeded, now check positively
				...
				} else if (o instanceof CharacterSet) { // match character set
		...
		if (ok && result != null && result.hasText()) { // sequence did match
														// and read non-null
														// text
			Token.Address end = new Token.Address(input.getScanLine(),
					input.getScanColumn(), input.getScanOffset());
			...
			return result; // null-text will be considered by consume() that
							// knows about nullable rules
		}
	...
	public String toString() {
		return hashCode()
				...
				+ ")> "
				+ toStringBase()
				+ (isNullable() && isRepeatable() ? " *" : isNullable() ? " ?"
						: isRepeatable() ? " +" : "");
	}
	...
	protected void listToString(List list, StringBuffer sb, String separator,
		...
		for (int i = 0; list != null && i < list.size(); i++) {
			...
			if (o instanceof Consumer)
				...
				// String or CharacterSet
				sb.append(o.toString());
		}
	}

	/**
	 * Returns true if the passed consumer could be concurrent with this. This
	 * method does not consider constraints, as these could be complex.
	 */
	public boolean overlaps(Consumer cc) {
		...
		if (o1 instanceof Consumer) { // drill down
			...
			} else if (o1 instanceof CharacterSet) {
				if (o2 instanceof CharacterSet)
					return ((CharacterSet) o1).overlaps((CharacterSet) o2);
				else if (o2 instanceof String)
					return ((CharacterSet) o1)
							.includes(((String) o2).charAt(0));
			} else if (o2 instanceof CharacterSet) {
				if (o1 instanceof String)
					return ((CharacterSet) o2)
							.includes(((String) o1).charAt(0));
			}
	...
	/**
	 * Marker class for references that must be resolved after building
	 * consumers.
	 */
	...
	private static class CharacterSet implements Serializable {
		...
		CharacterSet(String first, String last) throws LexerException {
			this.firstChar = first.charAt(0);
			this.lastChar = last.charAt(0);

			if (firstChar >= lastChar)
				throw new LexerException(
						"First character is bigger equal last: " + toString());
		}
		...
		/**
		 * Returns true if passed character set contains characters contained in
		 * this set.
		 */
		public boolean overlaps(CharacterSet other) {
			return other.includes(firstChar) || other.includes(lastChar)
					|| includes(other.firstChar) || includes(other.lastChar);
		}
		...
		public String toString() {
			if (stringRepres == null) {
				if (Character.isISOControl(firstChar)
						|| Character.isISOControl(lastChar))
					this.stringRepres = Integer.toHexString(firstChar) + ".."
