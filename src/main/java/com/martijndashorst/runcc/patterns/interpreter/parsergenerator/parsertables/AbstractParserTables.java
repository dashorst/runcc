 ...
 * <p>
 * Source generation method: <i>toSourceFile()</i>. Needed by
 * <i>CompiledTables</i>, used for buffering compiled table classes at runtime.
 * <p>
...
public abstract class AbstractParserTables implements ParserTables,
	...
	/**
	 * The GOTO-table with all possible follow states. As much lines as states
	 * and columns as terminals and nonterminals are present.
	 */
	protected List gotoTable;

	/**
	 * The PARSE-ACTION table with SHIFT and REDUCE entries. As much lines as
	 * states and as much columns as terminals are present.
	 */
	protected List parseTable;

	/**
	 * Non-terminals and terminals, without EPSILON, with START-symbol: column
	 * header of GOTO-table.
	 */
	...
	/**
	 * Helper for the cell width of printed tables. Can be set manually if
	 * automatic result does not fit.
	 */
	...
	/**
	 * Implements ParserTables: returns String List of terminals, without
	 * EPSILON.
	 */
	@Override
	...
	/**
	 * Implements ParserTables: Returns a Map of the expected tokens for current
	 * state, contained in keySet iterator.
	 */
	@Override
	...
	/**
	 * Factory to build precompiled parser tables, or construct them from
	 * scratch. Used by <i>CompiledTables</i> and <i>SerializedTables</i>.
	 * 
	 * @param parserType
	 *            LALRParserTables, SLRParserTables, LRParserTables class.
	 * @param syntax
	...
	public static AbstractParserTables construct(Class parserType, Syntax syntax)
		...
		if (syntax == null) { // load compiled table
			...
			try { // if this fails
				constr = parserType.getConstructor(new Class[] { syntax
						.getClass() });
			} catch (Exception e) { // try Object as parameter for Constructor
				constr = parserType
						.getConstructor(new Class[] { Object.class });
			}
			return (AbstractParserTables) constr
					.newInstance(new Object[] { syntax });
		}
	}
	...
	/**
	 * Implements ParserTables: prints rules, FIRST/FOLLOW sets, syntax nodes
	 * (states), GOTO-table, PARSE-ACTION-table.
	 */
	@Override
	...
	protected String dumpRule(Rule rule, int i) {
		StringBuffer sb = new StringBuffer("(Rule " + i + ")  "
				+ rule.getNonterminal() + " : ");
	...
	protected void dumpTable(String title, List head, List table,
			PrintStream out) {
		out.println(title);
		...
		for (Iterator it = table.iterator(); it.hasNext(); i++) {
			...
			for (Iterator it2 = head.iterator(); it2.hasNext();) {
				...
				String digit = intg == null ? "-" : intg
						.equals(ParserTables.SHIFT) ? "SH" : intg
						.equals(ParserTables.ACCEPT) ? "AC" : intg.toString();
	...
	private void dumpCell(String digit, int width, boolean isLast,
			PrintStream out) {
	...
	private void fwrite(String line) throws IOException {
		f.write(line, 0, line.length());
	}

	/**
	 * Generates a compilable source file "parserClassName.java". If the class
	 * name contains package path, the file will be generated within the
	 * corresponding directory, else within working directory.
	 * 
	 * @param parserClassName
	 *            name of class to generate: "Java" will generate "Java.java" in
	 *            current directory, "my.pkg.PT" will generate "my/pkg/PT.java"
	 *            in current directory.
	 * @return fileName if File could be written, else null.
	 */
	public String toSourceFile(String parserClassName) throws IOException {
		if (parserClassName.endsWith("ParserTables") == false)
			parserClassName = parserClassName + "ParserTables";

		String fileName = parserClassName.replace('.', File.separatorChar)
				+ ".java";
		...
		if (path != null) {
			...
			parserClassName = parserClassName.substring(parserClassName
					.lastIndexOf(".") + 1);
		}
		...
		fwrite(" */\n\n");
		fwrite("public final class " + parserClassName
		...
		for (int i = 0; i < syntax.size(); i++) {
			...
			fwrite("		syntax.addRule(s = new Rule(\"" + s.getNonterminal()
					+ "\", " + s.rightSize() + "));	// rule " + i + "\n");
		...
		fwrite("		terminalsWithoutEpsilon = new ArrayList("
				+ terminalsWithoutEpsilon.size() + ");\n");
		for (int i = 0; i < terminalsWithoutEpsilon.size(); i++)
			fwrite("		terminalsWithoutEpsilon.add(\""
					+ sub(terminalsWithoutEpsilon.get(i)) + "\");\n");
		...
		// every goto state is a method as Java can not load methods bigger than
		// 65563 Bytes
		for (int i = 0; i < gotoTable.size(); i++) {
			Map g = (Map) gotoTable.get(i);
			if (g != null) {
		...
		// every action state is a method as Java can not load methods bigger
		// than 65563 Bytes
		for (int i = 0; i < parseTable.size(); i++) {
			Map p = (Map) parseTable.get(i);
			if (p != null) {
