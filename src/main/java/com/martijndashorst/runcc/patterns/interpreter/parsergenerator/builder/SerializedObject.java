...
import com.martijndashorst.runcc.util.props.ConfigDir;
...
class SerializedObject {
	 ...
	 * from its basename (without any extension), else "Unknown" is assumed as
	 * basename. The returned name has no path.
	 */
	...
	/**
	 * Converts a File, InputStream, Reader, String, StringBuffer, List of Lists
	 * or String[][] to a Syntax.
	 */
	public static Syntax toSyntax(Object syntaxInput) throws Exception {
		Syntax syntax;
		if (syntaxInput instanceof Syntax)
			syntax = (Syntax) syntaxInput;
		else if (syntaxInput instanceof String[][])
			syntax = new Syntax((String[][]) syntaxInput);
		else if (syntaxInput instanceof List)
	...
	/**
	 * Creates the directory of passed filename if it does not exist. @return
	 * the directory name.
	 */
