...
/**
 * A semantic that builds a syntax tree model from a parser run. Every node of
 * the result tree contains the rule and the parsed token list. The result of
 * the parser run can be retrieved by <i>(TreeBuilderSemantic.Node)
 * parser.getResult()</i>.
 * 
...
public class TreeBuilderSemantic implements Semantic {
	/**
	 * Implements Semantic to store every node of the syntax tree with rule and
	 * values.
	 */
	@Override
	...
	/**
	 * Node class that representy a syntax tree node. All sub-nodes are
	...
	public static class Node {
		...
		/**
		 * Returns the human readable representation of this node and all
		 * sub-nodes.
		 * 
		 * @param indent
		 *            spaces to indent on sub-nodes.
		 */
