...
/**
	...
	@see com.martijndashorst.runcc.patterns.interpreter.parsergenerator.lexer.LexerImpl
...
public interface Lexer {
	...
	/**
	 * Tells the Lexer the terminals (tokens) to scan, called on init. Every
	 * terminal is a String that satisfies the facts defined in
	 * <i>Token.isTerminal()</i> (EPSILON or delimited by quotes).
	 * 
	 * @param terminals
	 ...
	 * parser hints, or by using contained character consumers, which are
	 * obtained by the lexer strategy.
	 * 
	 * @param tokenSymbols
	 *            a Map that contains token symbols (in "key" field) expected by
	 *            the Parser, can be null (slower).
	...
	/**
	 * A way to receive every parsing syntax Token the Lexer reads, even it is
	 * ignored. The implementer can install itself by calling
	 * <i>addTokenListener()</i>.
	 */
	public interface TokenListener {
		/**
		 * The implementer receives every token the lexer reads, even if it is
		 * ignored.
		 * 
		 * @param token
		 *            the Token that was successfully scanned from input.
		 * @param ignored
		 *            true when this Token will be thrown away (not passed to
		 *            Parser), else false.
		 */
	...
	/**
	 * Installs a TokenListener that wants to know about every read Token, even
	 * it is ignored.
	 * 
	 * @param listener
	 *            the Lexer.Listener implementation to install.
	 */
	 ...
	 * 
	 * @param listener
	 *            the Lexer.Listener implementation to remove.
	 */
