...
/**
 * Lexer-Parser communication struct. Utility methods defining EPSILON and
 * terminals. Definition of all global constants for parsergenerator packages.
 * <p>
 * This class defines special token identifiers: "token" (<i>Token.TOKEN</i>)
 * and "ignored" (<i>Token.IGNORED</i>), which are needed when specifiying a
 * syntax with mixed parser and lexer rules.
 * <p>
 * This class defines special symbols needed to define character sets with
 * <i>Token.UPTO</i> (e.g. A..Z) and intersections with <i>Token.BUTNOT</i>
 * (e.g. `char` - `newline`).
 * 
...
public class Token {
	/**
	 * This special token symbol means "end of input" to the Parser, which stops
	 * calling <i>getNextToken()</i> then.
	 */
	...
	/**
	 * Symbol used to define lexer character set intersections, e.g. <i>char -
	 * "/*" - "//"</i>.
	 */
	public static final String BUTNOT = "-";

	/**
	 * Reserved identifier that marks nonterminals the lexer should deliver to
	 * parser.
	 */
	...
	/**
	 * The character used to mark artificial nonterminal (e.g. made from
	 * "prolog?"). Every artificial rule starts with it.
	 */
	...
	/**
	 * Address stores input line number (1-n), column (0-n) and character/byte
	 * offset (0-n).
	 */
	...
	public static class Range implements Comparable<Range> {
		/**
		 * The start Address of this Token, pointing to the first
		 * character/byte.
		 */
		...
		@Override
		public boolean equals(Object o) {
			return start.equals(((Range) o).start)
					&& end.equals(((Range) o).end);
		}
		...
		public int compareTo(Range o) {
			return start.compareTo(o.start) + end.compareTo(o.end);
		}
	}
	...
	/**
	 * Distinction of 'terminals' and nonterminals: terminals are either
	 * starting with digit or are enclosed in quotes '"` or equal to EPSILON.
	 */
	public static boolean isTerminal(String symbol) {
		char c = symbol.charAt(0);
		return c == STRING_QUOTE || c == CHAR_QUOTE || c == COMMAND_QUOTE
				|| Character.isDigit(c) || Token.isEpsilon(symbol);
	}
}
