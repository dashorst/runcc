package com.martijndashorst.runcc.patterns.interpreter.parsergenerator.parsertables;
...
import com.martijndashorst.runcc.patterns.interpreter.parsergenerator.syntax.Rule;
import com.martijndashorst.runcc.patterns.interpreter.parsergenerator.syntax.Syntax;

/**
 * SLR bottom-up parser syntax node. The build() method of an no-arg-constructed
 * node is used to fill a state node list. The empty list passed to build() will
 * be filled with all state nodes after.
 * <p>
 * Every syntax node provides a fill() method to populate the corresponding line
 * (node-index) within PARSE-ACTION and GOTO tables.
 * <p>
 * After construction the syntax node has state entries represented by
 * RuleStateItem instances.
 * <p>
 * The state of a rule is represented by a dot "." at start, between symbols, or
 * at end. To shift an item means to move the dot to left.
 * 
...
class SLRSyntaxNode {
	...
	/**
	 * Factory-method to create a SLRSyntaxNode, to be overridden by other node
	 * types.
	 */
	...
	/**
	 * Factory-method to create a RuleStateItem, to be overridden by other node
	 * types.
	 */
	...
	/**
	 * Construction of start node and all further state nodes. After this call
	 * the syntaxNodes list is filled with all state-bound nodes.
	 * 
	 * @param syntax
	 *            the grammar, including START rule.
	 * @param syntaxNodes
	 *            ampty list that holds all syntax nodes after this call.
	 * @param kernels
	 *            empty hashtable for internal buffering.
	...
	public List build(Syntax syntax, List syntaxNodes, Hashtable kernels) {
		...
		generateSyntaxNodes(syntaxNodes, syntax, kernels); // generate all other
															// nodes
	...
	/**
	 * Generates syntax nodes into passed list, whereby every node represents
	 * one possible state. Every generated node gets appended to list and can
	 * generate new nodes by itself.
	 * 
	 * @param syntaxNodes
	 *            list of nodes
	 * @param syntax
	 *            the grammar rules
	 */
	protected void generateSyntaxNodes(List syntaxNodes, Syntax syntax,
			Hashtable kernels) {
	...
	/**
	 * Generates follower nodes from rule state items into list. The followers
	 * are referenced by their originators (GOTO-references).
	 * 
	 * @param syntaxNodes
	 *            list of nodes
	 * @param syntax
	 *            the grammar rules
	 */
	protected void generateSyntaxNodesFromItems(List syntaxNodes,
			Syntax syntax, Hashtable kernels) {
		for (Enumeration e = entries.elements(); e.hasMoreElements();) {
			RuleStateItem item = (RuleStateItem) e.nextElement();
			String pending = item.getPendingSymbol();

			if (item.followNodeIndex < 0 && pending != null) { // further states
																// are possible
				// create a kernel item
				SLRSyntaxNode node = createSyntaxNode();
				List tmp = node.addShiftedItems(pending, entries); // get
																	// entries
																	// that have
																	// been
																	// taken
	 ...
	 * as pending to the right of dot. This is done when a new node was
	 * generated, which happens when the dot is moved to the right. All adopted
	 * items together form the kernel of the syntax node. The number of kernel
	 * items is calculated, which is important when searching existing nodes.
	 * 
	 * @param symbol
	 *            the symbol that must be the pending symbol right of the dot
	 *            within rule state item
	 * @param originatorEntries
	 *            map containing all rule state items of the originator node (as
	 *            value).
	 * @return a Tuple list containing pairs of original and new item, the new
	 *         item is the shifted one.
	 */
	protected List addShiftedItems(String symbol, Hashtable originatorEntries) {
		...
		for (Enumeration e = originatorEntries.elements(); e.hasMoreElements();) {
			...
			if (pending != null && symbol.equals(pending)) {
				...
				list.add(new Tuple(item, newitem)); // return all derived
													// originator items
			}
		}
	...
	/**
	 * Store the follow state (node index) within rule state item. This is a
	 * sparate protected method as LALR nodes do further work here.
	 */
	protected void linkParentItemToChild(RuleStateItem parent, int newIndex,
			List syntaxNodes, RuleStateItem child) {
		parent.followNodeIndex = newIndex;
	}

	/**
	 * Closure - do for all rule states: Adopt all rules from grammar that
	 * derive one of the pending nonterminals (right of dot) within entries
	 * list. This is done recursively, appending new rules at end, so that they
	 * can adopt further rules. The closure method calls
	 * <i>addRulesDerivingPendingNonTerminal()</i>.
	 */
	...
	/**
	 * Closure call for one rule state item. All rules in grammar that have
	 * passed nonterm on left side get appended to todo list and put into item
	 * entries when not already in entries.
	 */
	protected void addRulesDerivingPendingNonTerminal(RuleStateItem item,
			String nonterm, Syntax syntax, List todo) {
	 ...
	 * 
	 * @param state
	 *            the index of this state within list
	 * @return Hashtable with all terminals/nonterminals to handle, and their
	 *         follower states.
	 */
	public Hashtable fillGotoLine(int state) {
		...
		for (Enumeration e = entries.elements(); e.hasMoreElements();) {
			...
			if (symbol != null) { // if pointer not at end of rule
				...
				setTableLine("GOTO", state, h, item, new Integer(
						item.followNodeIndex), symbol);
			}
		}
		return h;
	}
	 ...
	 *            the index of this state within list
	 * @param firstSets
	 *            all FIRST-sets of all nonterminals
	 * @param followSets
	 *            all FOLLOW-sets of all nonterminals
	 * @return Hashtable with all terminals to handle, and their actions.
	 */
	public Hashtable fillParseActionLine(int state, FirstSets firstSets,
		...
		for (Enumeration e = entries.elements(); e.hasMoreElements();) {
			...
			if (symbol != null) { // pointer not at end of rule, SHIFT
				if (Token.isTerminal(symbol)) { // enter SHIFT at terminal
												// symbol
					// first-set of terminal is terminal
					setParseTableLine(state, h, item, ParserTables.SHIFT,
							symbol);
				} else { // put SHIFT at all terminals of FIRST-set
					List firstSet = getNontermShiftSymbols(firstSets,
							item.getNonterminal());

					if (firstSet != null) { // LALR will return null, SLR not
											// null
						for (int i = 0; i < firstSet.size(); i++) {
							String terminal = (String) firstSet.get(i);
							setParseTableLine(state, h, item,
									ParserTables.SHIFT, terminal);
						}
					}
				}
			} else { // pointer at end, REDUCE to rule number
				for (Iterator reduceSymbols = getReduceSymbols(followSets, item); reduceSymbols
						.hasNext();) {
					String terminal = (String) reduceSymbols.next();

					if (item.ruleIndex == 0) // is startnode
						setParseTableLine(state, h, item, ParserTables.ACCEPT,
								terminal);
					else
						// ruleIndex > 0 means REDUCE
						setParseTableLine(state, h, item, new Integer(
								item.ruleIndex), terminal);
				}
			}
		}
		return h;
	...
	/**
	 * Returns all symbols for which SHIFT must be put into PARSE-ACTION table
	 * for a nonterminal. For SLR this is the FIRST set of the nonterminal.
	 */
	 ...
	 */
	protected Iterator getReduceSymbols(FollowSets followSets,
			RuleStateItem item) {
		return ((List) followSets.get(item.getNonterminal())).iterator();
	}

	/**
	 * Set a position in PARSE-ACTION table. This is the place where
	 * SHIFT/REDUCE and REDUCE/REDUCE conflicts are solved.
	 */
	protected void setParseTableLine(int state, Hashtable line,
			RuleStateItem item, Integer action, String terminal) {
		...
		if (setTableLine("PARSE-ACTION", state, line, item, action, terminal) == false) {
			...
			if (action.equals(ParserTables.SHIFT)
					|| o.equals(ParserTables.SHIFT)) {
				// prefer SHIFT operation
				line.put(terminal, ParserTables.SHIFT);
				System.err
						.println("WARNING: shift/reduce conflict, SHIFT is preferred.");
			} else {
				System.err
						.println("WARNING: reduce/reduce conflict, rule with smaller index is preferred.");
	...
	/**
	 * Set a position in one of the tables. Here SHIFT/SHIFT, SHIFT/REDUCE and
	 * REDUCE/REDUCE conflicts are detected.
	 * 
	 * @return true when no conflict was detected
	 */
	protected boolean setTableLine(String table, int state, Hashtable line,
			RuleStateItem item, Integer action, String terminal) {
		...
		if (o == null) { // no conflict
			...
			if (o.equals(action) == false) { // conflict!
				System.err
						.println("========================================================");
				System.err.println("WARNING: " + table + " state " + state
						+ ", terminal " + terminal + " is " + displayAction(o)
						+ " and was overwritten by action "
	...
	/**
	 * The count of kernel items must be equal. All kernel items must exist in
	 * passed node.
	 * 
	 * @param o
	 *            new node that contains only kernel items (which do not have
	 *            dot at start).
	 */
	@Override
	public boolean equals(Object o) {
		...
		for (Enumeration e = entries.elements(); e.hasMoreElements();) {
			...
			if (item.pointerPosition > 1
					&& node.entries.containsKey(item) == false)
				return false;
		}
		return true;
	}
	...
	/**
	 * Returns the hashcodes of all rule state items, associated by ^. The
	 * result gets buffered on first call.
	 */
	@Override
	public int hashCode() {
		if (hashCache == null) {
	...
	public String toString() {
		...
		for (Enumeration e = entries.elements(); e.hasMoreElements();) {
			...
			for (int i = 0; index == -1 && i < list.size(); i++) {
				...
				if (rsi1.ruleIndex > rsi.ruleIndex
						|| rsi1.ruleIndex == rsi.ruleIndex
						&& rsi.pointerPosition > 1)
					index = i;
			}
			if (index < 0)
				list.add(rsi);
	...
	protected class RuleStateItem {
		...
		/**
		 * Returns a new shifted rule state item (dot has moved one position
		 * right).
		 */
		RuleStateItem shift() {
		...
		/**
		 * Returns null when no nonterminal is after dot, else the nonterminal
		...
		public boolean equals(Object o) {
			...
			return ruleIndex == item.ruleIndex
					&& pointerPosition == item.pointerPosition;
		}
		...
		/**
		 * String representation of rule state, showing index, rule and dot
		 * position.
		 */
		@Override
		public String toString() {
			StringBuffer sb = new StringBuffer("(Rule " + ruleIndex + ") "
					+ getNonterminal() + " : ");
			int i = 0;
