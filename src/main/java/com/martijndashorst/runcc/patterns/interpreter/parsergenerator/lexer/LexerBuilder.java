 ...
 * contain character set symbols like ".." (set definitions) and "-"
 * (intersections).
 * <p>
 * The syntax may contain identifiers enclosed within `backquotes`. This marks
 * predefined lexer rules defined in <i>StandardLexerRules</i>. That class
 * contains default rules for numbers, identifiers, stringdefinitions,
 * characterdefinitions and many other (e.g. for XML), which can be used to
 * build lexers.<br>
 * <b>CAUTION:</b> Lexer and parser rules have the same namespace, you can not
 * define
 * 
 * <pre>
 ...
 * rules (tokens). When <b>token</b> is used, the builder does not try to
 * recognize any rule as lexer rule, so this must be good modeled. Be careful:
 * you can read away comments only by using <b>ignored</b>. But you can define
 * <b>ignored</b> without <b>token</b>, then nevertheless the builder tries to
 * recognize lexer rules.<br>
 * When the <b>token</b> marker is not used, the builder tries to separate lexer
 * from parser rules.
 * <p>
 * Example:
 * 
 ...
 * </pre>
 * 
 * Mind that the builder input can not be a text file, it must be wrapped into
 * <i>Syntax</i>. Use syntax builder to convert a text into a <i>Syntax</i>
 * object.
 * <p>
 * Java code fragment:
 * 
 ...
 * // when using the lexer standalone (without Parser), you must put the token
 * // terminal symbols into it now:
 * lexer.setTerminals(separation.getTokenSymbols());
 * </pre>
 * 
 * @see com.martijndashorst.runcc.patterns.interpreter.parsergenerator.syntax.builder.SyntaxSeparation
 * @see com.martijndashorst.runcc.patterns.interpreter.parsergenerator.lexer.StandardLexerRules
...
public class LexerBuilder {
	 ...
	 * 
	 * @param lexerSyntax
	 *            lexer rule (without token and ignored, use SyntaxSeparation
	 *            for that)
	 ...
	 * ...).
	 */
	public Lexer getLexer(Object input) throws IOException {
		Lexer lexer = getLexer();
		lexer.setInput(input);
		return lexer;
	}

	private void build(Syntax lexerSyntax) throws LexerException,
			SyntaxException {
		SyntaxSeparation.IntArray deleteIndexes = new SyntaxSeparation.IntArray(
				lexerSyntax.size());
		if (DEBUG)
		...
		// check for unresolved repeatable and nullable rules and delete them
		// from lexer syntax
		for (int i = 0; i < lexerSyntax.size(); i++) {
			...
			if (checkNullableRule(nonterm, rule, i, deleteIndexes) == false)
				if (checkRepeatableRule(nonterm, rule, i, deleteIndexes) == false)
					throw new LexerException(
							"Found no character consumer for nullable or repeatable rule "
									+ rule);
		}
		deleteIndexes.removeIndexesFrom(lexerSyntax);

		if (lexerSyntax.size() > 0) { // not all rules have been resolved to
										// character consumers
			throw new LexerException(
					"Could not process rules in lexer syntax: " + lexerSyntax);
		}
		...
		for (int i = 0; i < rule.rightSize(); i++) { // loop all symbols on
			...
			if (sym.equals(Token.BUTNOT)) {
				if (i == 0 || state != CONCATENATION)
					throw new LexerException(
							"Missing symbol to subtract from: " + rule);
				state = SUBTRACTION;
			} else if (sym.equals(Token.UPTO)) {
				if (i == 0 || state != CONCATENATION)
					throw new LexerException("Missing lower limit of set: "
							+ rule);
				state = SET;
			} else {
				String convertedSym = convertSymbol(sym); // remove quotes or
															// convert number to
															// char
				boolean isNonterm = convertedSym.equals(sym);
				if (isNonterm && state == SET)
					throw new LexerException(
							"Can not append nonterminal to set: " + rule);

				boolean setWillHappen = rule.rightSize() > i + 1
						&& rule.getRightSymbol(i + 1).equals(Token.UPTO); // next
																			// symbol
																			// will
																			// be
																			// ".."
				...
				if (state == SET) {
					...
					setConsumer = currentConsumer; // reset if intersection
													// happened
					...
					if (isNonterm)
						if (setWillHappen)
							throw new LexerException(
									"Nonterminal can not open set after subtraction: "
											+ rule);
						else
							currentConsumer
									.subtract(new Consumer.Reference(sym));
					else if (setWillHappen)
						currentConsumer.subtract(setConsumer = new Consumer(
								convertedSym));
					else
					...
					if (isNonterm)
						if (setWillHappen)
							throw new LexerException(
									"Nonterminal can not open set in concatenation: "
											+ rule);
						else
							currentConsumer.append(new Consumer.Reference(sym));
					else
						currentConsumer.append(convertedSym); // a following set
																// will be
																// recognized by
																// consumer
				}
	...
	private boolean checkNullableRule(String nonterm, Rule rule, int index,
			SyntaxSeparation.IntArray deleteIndexes) {
	...
	private boolean checkRepeatableRule(String nonterm, Rule rule, int index,
			SyntaxSeparation.IntArray deleteIndexes) {
		// We take away rules that are left recursive like
		// nonterm ::= nonterm something // this is the rule to remove now
		// nonterm ::= something // "nonterm" must be already among scanner
		// rules
	...
	/**
	 * Converts a character or string definition to its processable form. This
	 * implementation must be according to "bnf_chardef" in
	 * <i>StandardLexerRules</i>.
	 * <ul>
		...
		if (sym.charAt(0) == '\'' || sym.charAt(0) == '"') {
			...
			if (s.length() <= 0)
				throw new IllegalArgumentException(
						"Empty character or string definition: " + sym);

			StringBuffer sb = new StringBuffer(s.length()); // convert escape
															// sequences to
															// their real
															// meaning
			...
			char c;
			if (sym.startsWith("0x") || sym.startsWith("0X")) // hexadecimal
																// number
				...
				c = (char) Integer.valueOf(sym).intValue(); // will throw
