...
/**
 * Input for text lines. Line number, line position, current line and previous
 * line are available.
 * 
...
class InputText extends Input {
	...
	private List lineLengths = new ArrayList(); // list of Integer containing
												// line lengths including
												// newline sequence
	private int[] scanPoint;

	InputText(Object input) throws IOException {
		super(input);
	}
		...
		if (i == '\r') {
			...
			if (i == '\n') {
				if (wasCr == false) // on UNIX
					...
					// on WINDOWS, ignore \n after \r, but adjust previous line
					// length
					lineLengths.set(
							lineLengths.size() - 1,
							new Integer(((Integer) lineLengths.get(lineLengths
									.size() - 1)).intValue() + 1));
			} else {
				storeRead(i);
			}
	...
	private void newLine() {
		lineLengths.add(new Integer(column + 1)); // line length plus one for
													// the newline, will be
													// adjusted when \r\n
	...
	/**
	 * Returns the current read line number (1-n). This is always bigger equal
	 * than scan line.
	 */
	...
	/**
	 * Returns the current read position (0-n). This is different from scan
	 * position.
	 */
	...
	/**
	 * Returns the current scan line number (1-n). This is always smaller equal
	 * than read line.
	 */
	...
	/**
	 * Returns the current scan position (0-n). This is different from read
	 * position.
	 */
	...
	private int[] calculateScanPoint() { // returns line/column
		if (scanPoint != null) // use buffered scan pointer when possible, as
								// line/column requests always are coupled
			return scanPoint;
		...
		for (int i = lineLengths.size() - 1; i >= 0; i--) { // loop back all
															// lines
		...
		throw new IllegalStateException(
				"Something went wrong when calculating scan point: " + diff);
	}
