...
/**
 * LR bottom-up parser syntax node. This node type contains an algorithm to
 * propagate lookahead-sets of items to dependent items after syntax nodes were
 * constructed.
 * 
 * @see com.martijndashorst.runcc.patterns.interpreter.parsergenerator.parsertables.LRSyntaxNode
...
class LALRSyntaxNode extends LRSyntaxNode {
	...
	/**
	 * Factory-method that constructs a LALRRuleStateItem. A start-lookahead
	 * gets appended to the item when it is the start node.
	 */
	@Override
	 ...
	 * nonterminal. Default lookaheads are calculated here. Items that need
	 * lookahead propagation are located here.
	 */
	@Override
	protected void addRulesDerivingPendingNonTerminal(RuleStateItem itm,
		...
		for (int i = 0; i < syntax.size(); i++) {
			...
			if (rule.getNonterminal().equals(nonterm)) {
				LALRRuleStateItem rsi = (LALRRuleStateItem) createRuleStateItem(
						i, rule);

				// look if new item is already contained
				LALRRuleStateItem existing = (LALRRuleStateItem) entries
						.get(rsi);
				if (existing != null) {
					rsi = existing;
				...
				if (lookahead == null)
					// calculate lookahead of originator and if it needs to
					// propagate changes in
					// its lookahead to the new items (has nullable nonterms
					// until end)
					needsPropagation = item.calculateLookahead(
							lookahead = new ArrayList(), nullable, firstSets);
	...
	/**
	 * Rule state entry item class, contained within LALR syntax node. Lookahead
	 * propagation is implemented here.
	 */
	protected class LALRRuleStateItem extends LRRuleStateItem {
		...
		void propagateLookaheads(Iterator originatorLookahead) {
			...
			if (needsPropagation == false
					&& (originatorLookahead == null || originatorLookahead
							.hasNext() == false))
				return;
			...
			if (change || needsPropagation) {
				...
				for (int i = 0; i < propagateItems.size(); i++) {
					LALRRuleStateItem item = (LALRRuleStateItem) propagateItems
							.get(i);
