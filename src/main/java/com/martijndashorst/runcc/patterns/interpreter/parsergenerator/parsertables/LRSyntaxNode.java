 ...
 * 
 * @see com.martijndashorst.runcc.patterns.interpreter.parsergenerator.parsertables.SLRSyntaxNode
...
class LRSyntaxNode extends SLRSyntaxNode {
	...
	/**
	 * Construction of node with FIRST-sets and nullability of all nonterminals
	 * in syntax.
	 */
	...
	/**
	 * Factory-method that constructs a LRRuleStateItem. A start-lookahead gets
	 * appended to the item when it is the start node.
	 */
	@Override
	...
	/**
	 * Method called from closure, adopt all rules that derive the pending
	 * nonterminal. Default lookaheads are calculated here.
	 */
	@Override
	protected void addRulesDerivingPendingNonTerminal(RuleStateItem itm,
			String nonterm, Syntax syntax, List newItems) {
		...
		for (int i = 0; i < syntax.size(); i++) {
			...
			if (rule.getNonterminal().equals(nonterm)) {
				LRRuleStateItem rsi = (LRRuleStateItem) createRuleStateItem(i,
						rule);

				if (lookahead == null) // calculate lookahead, all new items get
										// the same lookahead
					item.calculateLookahead(lookahead = new ArrayList(),
							nullable, firstSets);
	 ...
	 * for a nonterminal. For LR and LALR this returns null.
	 */
	@Override
	...
	/**
	 * Rule state entry item class, contained within LR syntax node. Adds
	 * calculation and adoption of lookaheads functionality to super class.
	 */
	protected class LRRuleStateItem extends RuleStateItem {
		 ...
		 * newLookahead argument. These go to lookahead: FIRST set of second
		 * symbol after the dot (terminal is taken itself), and all FIRST sets
		 * of following symbols as long as they are nullable (terminal is not
		 * nullable). When symbols all were nullable, the lookahead of this item
		 * also goes to lookahead.
		 * 
		 * @return true when all symbols after the dot in the rule were
		 *         nullable. In LALR the originator item then propagates its
		 *         lookaheads to this one.
		 */
		boolean calculateLookahead(List newLookahead, Nullable nullable,
				FirstSets firstSets) {
			// consider all nullable symbols after the one past the dot
			// and add their first symbols to lookahead set.

			for (int i = pointerPosition; i < rule.rightSize(); i++) { // when
																		// pointer
																		// at
																		// start,
																		// it
			...
			// if we get here everything was nullable, add all lookaheads of
			// this item
		...
		/**
		 * All lookahead hashcodes are associated by ^, plus rule index * 13 +
		 * dot position.
		 */
		@Override
		public int hashCode() {
		...
		public String toString() {
			...
			int i = s.lastIndexOf("->");
			if (i > 0)
				s = s.substring(0, i) + "LOOKAHEAD" + hashToStr(lookahead)
						+ "	" + s.substring(i);
			else
