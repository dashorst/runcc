...
import com.martijndashorst.runcc.patterns.interpreter.parsergenerator.lexer.LexerSemantic;
import com.martijndashorst.runcc.patterns.interpreter.parsergenerator.lexer.ResultTree;
...
import com.martijndashorst.runcc.util.io.UnicodeReader;
...
public class XmlLexer {
	public static void main(String[] args) throws Exception {
		if (args.length <= 0) {
			System.err.println("SYNTAX: java " + XmlLexer.class.getName()
					+ " file.xml [file.xml ...]");
		...
		// Building lexer from scratch takes 840 millis. Parsing takes 60 millis
		// for a 70 line XML file.

		// read the syntax from EBNF file
		Reader syntaxInput = new InputStreamReader(
				XmlLexer.class.getResourceAsStream("Xml.syntax"));
		boolean PRODUCTION = false; // always build from scratch at development
									// time
		LexerImpl lexer = (LexerImpl) new SerializedLexer(PRODUCTION).get(
				syntaxInput, "Xml");

		System.err.println("time to build XML file parser was "
				+ timer.getInterval());

		for (int i = 0; i < args.length; i++) {
			String parseFile = args[i];
			Reader parseInput = new UnicodeReader(
	...
	static class PrintXmlLexerSemantic implements LexerSemantic {
		...
		public void ruleEvaluated(Rule rule, ResultTree resultTree) {
			System.out.println("Nonterminal=" + rule.getNonterminal()
					+ ", range(" + resultTree.getRange() + "), Input=\""
					+ resultTree.toString() + "\"");
		}

		/**
		 * Returns a Set of nonterminal Strings whose rule evaluations the Lexer
		 * should report to this semantic. Could return null to receive all
		 * rules. For XML only a subset of all tokens in the EBNF is needed.
		 * There is no other way than to hardcode those nonterminal names here.
		 * When using the SourceGenerator on the XML EBNF, the Strings could be
