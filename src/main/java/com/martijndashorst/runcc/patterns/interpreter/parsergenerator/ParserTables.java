...
public interface ParserTables {
	/**
	 * Special symbol occuring in a parser table, 0, means: syntax was correct
	 * when coming to that cell.
	 */
	public static final Integer ACCEPT = new Integer(0);

	/**
	 * Special symbol occuring in a parser table, -1, means: error when coming
	 * to that cell.
	 */
	...
	/**
	 * Returns the next state from the GOTO-table, for a given state and a
	 * received terminal or nonterminal.
	 * 
	 * @param currentState
	...
	/**
	 * Returns the action from the PARSE-ACTION-table, for a given state and
	 * received terminal. fuer einen gegebenen Zustand und ein Terminal.
	 * 
	 * @param currentState
	 *            the current parser state
	 * @param terminal
	 *            recently received terminal
	 * @return new action to proceed, SHIFT, ACCEPT, ERROR, or anything above
	 *         zero, meaning REDUCE.
	 */
	...
	/**
	 * The keySet of returned Map contains all expected terminals for the
	 * current state. These can be used as Lexer hints.
	 */
