...
/**
 * Calculator for arithmetic expressions, showing the elegance of
 * ReflectSemantic.
 * <p>
 * Syntax: java fri.patterns.interpreter.parsergenerator.examples.Calculator
 * '(4+2.3) *(2 - -6) + 3*2'
 * 
...
public class Calculator extends ReflectSemantic {
	...
	public Object EXPRESSION(Object EXPRESSION, Object operator, Object TERM) {
		if (operator.equals("+"))
			return new Double(((Double) EXPRESSION).doubleValue()
	...
	public Object TERM(Object TERM, Object operator, Object FACTOR) {
		if (operator.equals("*"))
			return new Double(((Double) TERM).doubleValue()
	...
	public Object FACTOR(Object leftParenthesis, Object EXPRESSION,
			Object rightParenthesis) {
		return EXPRESSION;
	}
	...
	public static void main(String[] args) throws Exception {
		if (args.length <= 0) {
			System.err.println("SYNTAX: java " + Calculator.class.getName()
					+ " \"(4+2.3) *(2 - -6) + 3*2\"");
			System.exit(1);
		}
		...
		System.err.println("Parse return " + ok + ", result: "
				+ parser.getResult());

		/*
		 * Variant without SerializedParser: SyntaxSeparation separation = new
		 * SyntaxSeparation(new Syntax(rules)); // takes away IGNORED
		 * LexerBuilder builder = new LexerBuilder(separation.getLexerSyntax(),
		 * separation.getIgnoredSymbols()); Lexer lexer =
		 * builder.getLexer(input); ParserTables parserTables = new
		 * LALRParserTables(separation.getParserSyntax()); Parser parser = new
		 * Parser(parserTables); boolean ok = parser.parse(lexer, new
		 * Calculator());
		 * System.err.println("Parse return "+ok+", result: "+parser
		 * .getResult());
		 */
	}
}
