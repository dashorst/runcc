 ...
 * e.g. `cstylecomment`.
 * <p>
 * Furthermore it provides methods to retrieve sets of rules describing certain
 * standard scan items like `number` or `identifier`. The resulting arrays can
 * be built together by <i>SyntaxUtil.catenizeRules(...)</i>.
 * <p>
 * This class provides rules for comments with an arbitrary start character or
 * start/end sequence:
 * <ul>
 ...
 * 		{ Token.TOKEN, &quot;`stringdef`&quot; }, // need this rule as string definitions
 * 										// could contain comments
 * 		{ Token.IGNORED, &quot;`cstylecomment`&quot; },
 * 		{ &quot;others&quot;, &quot;others&quot;, &quot;other&quot; },
 * 		{ &quot;others&quot;, &quot;other&quot; },
 * 		{ &quot;other&quot;, &quot;`char`&quot;, Token.BUTNOT, &quot;`cstylecomment`&quot;, Token.BUTNOT,
 * 				&quot;`stringdef`&quot; }, };
...
public abstract class StandardLexerRules {
	/**
	 * Returns e.g. the Letter-Rules <i>getUnicodeLetterRules()</i> for id
	 * "letter". Using this, one can write things like `identifier` in a Lexer
	 * specification text, as LexerBuilder tries to resolve these words calling
	 * this method. Possible values for id are:
	 * <ul>
	 ...
	 * <li>stringdef</li>
	 * <li>chardef</li>
	 * <li>bnf_chardef (differs as in BNF characters can be written as "020"
	 * instead of '\020')</li>
	...
	/**
	 * Returns rules for a custom comment (like C-style "//", but with passed
	 * start sequence).
	 * 
	 * @param nonterminalName
	 *            name of comment to be used within syntax, e.g. "basicComment".
	 * @param startChar
	 *            string (1-n characters) defining the start sequence of the
	 *            comment, e.g. ";"
	 */
	public static final String[][] getCustomOneLineCommentRules(
			String nonterminalName, String startChar) {
		...
		customRule[1] = "\"" + startChar + "\""; // put custom sequence where
													// where "//" sits
		sarr2[0] = customRule;
	 ...
	 * 
	 * @param nonterminalName
	 *            name of comment to be used within syntax, e.g.
	 *            "pascalComment".
	 * @param startSeq
	 *            string defining the start sequence of the comment, e.g. "(*"
	 * @param endSeq
	 *            string defining the end sequence of the comment, e.g. "*)"
	 */
	public static final String[][] getCustomMultiLineCommentRules(
			String nonterminalName, String startSeq, String endSeq) {
		String[][] sarr0 = getUnicodeCharRules();
		String[][] sarr1 = getNewlineRules();
		String[][] customRules = new String[6][];
		customRules[0] = new String[] { nonterminalName,
				"\"" + startSeq + "\"", "char_minus_star_slash_list_opt",
				"\"" + endSeq + "\"" };
		customRules[1] = new String[] { "char_minus_" + nonterminalName,
				"char", Token.BUTNOT, "\"" + endSeq + "\"" };
		customRules[2] = new String[] {
				"char_minus_" + nonterminalName + "_list",
				"char_minus_" + nonterminalName + "_list",
				"char_minus_" + nonterminalName };
		customRules[3] = new String[] {
				"char_minus_" + nonterminalName + "_list",
				"char_minus_" + nonterminalName };
		customRules[4] = new String[] {
				"char_minus_" + nonterminalName + "_list_opt",
				"char_minus_" + nonterminalName + "_list" };
		customRules[5] = new String[] { "char_minus_" + nonterminalName
				+ "_list_opt" /* nothing */};
	...
	/**
	 * Rules to scan identifiers that start with letter and continue with letter
	 * or digit or '_'.
	 */
	...
	/**
	 * Rules to scan BNF-like 'c'haracterdefinitions. They differ from
	 * C/Java-chardefs in that they can be written as digits: 0x20.
	 */
	...
	/**
	 * Rules to scan "stringdefinitions" that can contain backslash as masking
	 * character.
	 */
	...
	/**
	 * Rules to scan C-style slash-star and slash-slash AND shell-style #
	 * comments.
	 */
	...
	public static void printRules(String[][] syntax) {
		for (int i = 0; i < syntax.length; i++) {
			for (int j = 0; j < syntax[i].length; j++)
				System.out.print(j == 1 ? " ::= " + syntax[i][j] + " "
						: syntax[i][j] + " ");
			System.out.println();
		}
	}
	...
	/**
	 * Premade lexer syntax used to scan textual EBNF-like syntax
	 * specifications.
	 */
	public static final String[][] lexerSyntax = {
			...
			// bell \a was removed in favor of backslash and too much work
			// re-numbering ...
			/* 258 */{ "bnf_chardef", "digits" },

			/* 259 */{ "identifier", "letter_or_uscore",
					"letter_or_digit_list_opt" },
			/* 260 */{ "letter_or_uscore", "letter" },
			/* 261 */{ "letter_or_uscore", "'_'" },
			/* 262 */{ "letter_or_digit", "letter_or_uscore" },
			/* 263 */{ "letter_or_digit", "digit" },
			/* 264 */{ "letter_or_digit_list", "letter_or_digit_list",
					"letter_or_digit" },
			...
			/* 269 */{ "char_minus_doublequote_list",
					"char_minus_doublequote_list", "char_minus_doublequote" },
			/* 270 */{ "char_minus_doublequote_list", "char_minus_doublequote" },
			/* 271 */{ "char_minus_doublequote", "char", Token.BUTNOT, "'\"'",
					Token.BUTNOT, "'\\'" }, // does not contain " or \
			/* 272 */{ "char_minus_doublequote_list_opt",
					"char_minus_doublequote_list" },
			/* 273 */{ "char_minus_doublequote_list_opt" /* nothing */},
			/* 274 */{ "backslash_char_list", "backslash_char_list",
					"backslash_char" },
			/* 275 */{ "backslash_char_list", "backslash_char" },
			/* 276 */{ "backslash_char", "'\\'", "char" }, // escaped character
															// like "\""
			/* 277 */{ "backslash_char_list_opt", "backslash_char_list" },
			/* 278 */{ "backslash_char_list_opt" /* nothing */},
			/* 279 */{ "stringpart", "char_minus_doublequote_list_opt",
					"backslash_char_list_opt" },
			...
			/* 284 */{ "cstylecomment", "\"/*\"",
					"char_minus_star_slash_list_opt", "\"*/\"" },
			/* 285 */{ "char_minus_star_slash", "char", Token.BUTNOT, "\"*/\"" },
			/* 286 */{ "char_minus_star_slash_list",
			...
			/* 292 */{ "char_minus_newline_list", "char_minus_newline_list",
					"char_minus_newline" },
			...
			/* 296 */{ "shellstylecomment", "'#'",
					"char_minus_newline_list_opt" },

			/* 297 */{ "ruleref", "'" + Token.COMMAND_QUOTE + "'",
					"identifier", "'" + Token.COMMAND_QUOTE + "'" },
	...
	/**
	 * Numerical rules for <b>numbers</b> within sourcecode: number ::= integer
	 * | float.
	 */
