 ...
 * <p>
 * An artifical START-node gets inserted. Lists of nonterminals and terminals
 * are created. Syntax nodes are created, from which the parser GOTO and
 * PARSE-ACTION tables get built.
 * <p>
...
public class SLRParserTables extends AbstractParserTables {
	...
	/**
	 * Appends START symbol and retrieves all symbols. Calls init then. All
	 * parser table types run through this constructor.
	 */
	...
	protected void init() throws ParserBuildException {
		syntaxNodes = new SLRSyntaxNode().build(syntax, syntaxNodes,
				new Hashtable());
	...
	private Syntax addStartSymbol(Syntax syntax) throws ParserBuildException {
		...
		if (startRules.size() <= 0) { // no toplevel rule
			// throw new
			// ParserBuildException("Grammatik hat keine Start-Regel!");
			Rule rule = syntax.getRule(0);
			System.err
					.println("WARNING: Grammar has no top level rule, taking first rule >"
							+ rule + "<");
			startSym = rule.getNonterminal();
		} else if (startRules.size() > 1) { // more than one toplevel rules
			for (int i = 0; i < startRules.size(); i++) { // check if all start
															// rules have the
															// same nonterminal
															// on left side
				...
				if (startSym == null)
					...
					throw new ParserBuildException(
							"Grammar has more than one toplevel rules: "
									+ startRules);
			}
	...
	/**
	 * Returns all symbols (terminals and nonterminals). Builds lists. Called
	 * only once on construction.
	 */
	protected List getAllSymbols() throws ParserBuildException {
		...
		for (int i = 0; i < syntax.size(); i++) {
			...
			if (Nullable.isNull(nonterm))
				throw new ParserBuildException("ERROR: Empty nonterminal: >"
						+ nonterm + "<");
		...
		for (int j = 0; j < syntax.size(); j++) {
			...
			for (int i = 0; i < rule.rightSize(); i++) {
				...
				if (Nullable.isNull(symbol))
					throw new ParserBuildException("ERROR: Empty terminal: >"
							+ symbol + "<");
				...
				if (nonterminals.indexOf(symbol) < 0)
					throw new ParserBuildException(
							"ERROR: Every nonterminal must have a rule. symbol: >"
									+ symbol + "<, rule: " + rule);
			}
		}
		...
		// check for sanity
		if (terminals.size() <= 0)
			throw new ParserBuildException("ERROR: No terminal found: "
					+ syntax);

		if (nonterminals.size() <= 0)
			throw new ParserBuildException("ERROR: No nonterminal found: "
					+ syntax);
	 ...
	 */
	protected void insertTableLine(int i, Map line, List table, Map hash) {
	...
	/**
	 * Enable garbage collection of builder variables. CAUTION: dump methods
	 * work reduced after this call.
	 */
	...
	public void report(PrintStream out) {
		...
		super.report(out);
		out.println("states: "
				+ (syntaxNodes != null ? syntaxNodes.size() : -1));
	}

	/**
	 * Implements ParserTables: output of rules, FIRST/FOLLOW sets, syntax
	 * nodes, GOTO-table, PARSE-ACTON-table.
	 */
	@Override
	...
	public static void main(String[] args) {
		String[][] syntax = { { "EXPR", "TERM" },
				{ "EXPR", "EXPR", "'+'", "TERM" },
				{ "EXPR", "EXPR", "'-'", "TERM" }, { "TERM", "FAKT", },
				{ "TERM", "TERM", "'*'", "FAKT" },
				{ "TERM", "TERM", "'/'", "FAKT" }, { "FAKT", "`number`", },
				{ "FAKT", "'('", "EXPR", "')'" }, };
 ...
 * ["[0-9]+", '('] FIRST(TERM) = ["[0-9]+", '(']
 * 
 * FOLLOW(FAKT) = [Epsilon, '+', '-', ')', '*', '/'] FOLLOW(EXPR) = [Epsilon,
 * '+', '-', ')'] FOLLOW(<START>) = [Epsilon] FOLLOW(TERM) = [Epsilon, '+', '-',
 * ')', '*', '/']
 * 
 * State 0 (Rule 0) <START> : .EXPR -> State 2 (Rule 1) EXPR : .TERM -> State 1
 * (Rule 2) EXPR : .EXPR '+' TERM -> State 2 (Rule 3) EXPR : .EXPR '-' TERM ->
 * State 2 (Rule 4) TERM : .FAKT -> State 4 (Rule 5) TERM : .TERM '*' FAKT ->
 * State 1 (Rule 6) TERM : .TERM '/' FAKT -> State 1 (Rule 7) FAKT : ."[0-9]+"
 * -> State 5 (Rule 8) FAKT : .'(' EXPR ')' -> State 3
 * 
 * State 1 (Rule 1) EXPR : TERM . (Rule 5) TERM : TERM .'*' FAKT -> State 7
 * (Rule 6) TERM : TERM .'/' FAKT -> State 6
 * 
 * State 2 (Rule 0) <START> : EXPR . (Rule 2) EXPR : EXPR .'+' TERM -> State 9
 * (Rule 3) EXPR : EXPR .'-' TERM -> State 8
 * 
 * State 3 (Rule 1) EXPR : .TERM -> State 1 (Rule 2) EXPR : .EXPR '+' TERM ->
 * State 10 (Rule 3) EXPR : .EXPR '-' TERM -> State 10 (Rule 4) TERM : .FAKT ->
 * State 4 (Rule 5) TERM : .TERM '*' FAKT -> State 1 (Rule 6) TERM : .TERM '/'
 * FAKT -> State 1 (Rule 7) FAKT : ."[0-9]+" -> State 5 (Rule 8) FAKT : '('
 * .EXPR ')' -> State 10 (Rule 8) FAKT : .'(' EXPR ')' -> State 3
 * 
 * State 4 (Rule 4) TERM : FAKT .
 * 
 * State 5 (Rule 7) FAKT : "[0-9]+" .
 * 
 * State 6 (Rule 6) TERM : TERM '/' .FAKT -> State 11 (Rule 7) FAKT : ."[0-9]+"
 * -> State 5 (Rule 8) FAKT : .'(' EXPR ')' -> State 3
 * 
 * State 7 (Rule 5) TERM : TERM '*' .FAKT -> State 12 (Rule 7) FAKT : ."[0-9]+"
 * -> State 5 (Rule 8) FAKT : .'(' EXPR ')' -> State 3
 * 
 * State 8 (Rule 3) EXPR : EXPR '-' .TERM -> State 13 (Rule 4) TERM : .FAKT ->
 * State 4 (Rule 5) TERM : .TERM '*' FAKT -> State 13 (Rule 6) TERM : .TERM '/'
 * FAKT -> State 13 (Rule 7) FAKT : ."[0-9]+" -> State 5 (Rule 8) FAKT : .'('
 * EXPR ')' -> State 3
 * 
 * State 9 (Rule 2) EXPR : EXPR '+' .TERM -> State 14 (Rule 4) TERM : .FAKT ->
 * State 4 (Rule 5) TERM : .TERM '*' FAKT -> State 14 (Rule 6) TERM : .TERM '/'
 * FAKT -> State 14 (Rule 7) FAKT : ."[0-9]+" -> State 5 (Rule 8) FAKT : .'('
 * EXPR ')' -> State 3
 * 
 * State 10 (Rule 2) EXPR : EXPR .'+' TERM -> State 9 (Rule 3) EXPR : EXPR .'-'
 * TERM -> State 8 (Rule 8) FAKT : '(' EXPR .')' -> State 15
 * 
 * State 11 (Rule 6) TERM : TERM '/' FAKT .
 * 
 * State 12 (Rule 5) TERM : TERM '*' FAKT .
 * 
 * State 13 (Rule 3) EXPR : EXPR '-' TERM . (Rule 5) TERM : TERM .'*' FAKT ->
 * State 7 (Rule 6) TERM : TERM .'/' FAKT -> State 6
 * 
 * State 14 (Rule 2) EXPR : EXPR '+' TERM . (Rule 5) TERM : TERM .'*' FAKT ->
 * State 7 (Rule 6) TERM : TERM .'/' FAKT -> State 6
 * 
 * State 15 (Rule 8) FAKT : '(' EXPR ')' .
 * 
 * 
