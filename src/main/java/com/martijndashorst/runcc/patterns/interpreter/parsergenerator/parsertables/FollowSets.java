...
import com.martijndashorst.runcc.util.collections.UniqueAggregatingHashtable;

/**
 * Map of all FOLLOW-sets of a SLR syntax. Key is nonterminal.
 * <p>
 * The FOLLOW-set for a nonterminal is the collection of terminals that can
 * appear behind that nonterminal on the right side of all rules.
 * <p>
 * <b>Algorithm:</b><br>
 * Search all rules that contain the nonterminal on right side. Scan for a
 * terminal after the nonterminal on right side until found or one scanned
 * nonterminal is not nullable. The FIRST set of every scanned nonterminal goes
 * to the FOLLOW set. When reaching the end without having found a terminal or a
 * non-nullable nonterminal, even the FOLLOW set of the nonterminal on the left
 * side of that rule goes to FOLLOW.
 * 
...
class FollowSets extends UniqueAggregatingHashtable {
	 ...
	 * The FOLLOW sets will then be Lists within a Map that provides the
	 * nonterminal as key.
	 * 
	 * @param syntax
	 *            the grammar
	 * @param nullAble
	 *            Map containing Boolean true when nonterminal (key) is
	 *            nullable, else false.
	 * @param firstSets
	 *            ready-made FIRST sets for all nonterminals.
	 */
	public FollowSets(Syntax syntax, Nullable nullAble, FirstSets firstSets)
			throws ParserBuildException {
		// collect FOLLOW sets into a Hashtable where each nonterminal has a
		// list of FOLLOW symbols
	...
	private void generateFollow(Syntax syntax, Nullable nullAble,
			FirstSets firstSets) {
		...
		for (int rulePos = 0; rulePos < syntax.size(); rulePos++) {
			...
			if (rulePos == 0) { // START node, FOLLOW of start node is always
								// epsilon
				...
				List nonterms = new ArrayList(); // collect receiving
													// nonterminals
				...
				for (int i = 0; i < rule.rightSize(); i++) {
					...
					if (nonterms.size() > 0) { // if there are receivers (not at
												// first element)
						// add symbol to FOLLOW sets of all nonterminals in
						// local list
						List l;
						if (isTerminal) {
							l = new ArrayList();
							...
							l = (List) firstSets.get(symbol); // FIRST(symbol)
																// goes to
																// FOLLOW set
						}
					...
					if (isTerminal || nullAble.isNullable(symbol) == false) { // if
																				// it
																				// is
																				// a
																				// terminal
																				...
																				// be
																				// null
						nonterms.clear(); // empty list of receiving
											// nonterminals
					}
					...
					if (isTerminal == false) { // if it is a nonterminal
						...
						if (i == rule.rightSize() - 1) { // at end
							// add left side of rule to FOLLOW sets of all
	...
	private void resolveSets() throws ParserBuildException {
		// Make plain lists of mixed reference lists: all nonterminals are
		// resolved to their terminal lists
		...
		for (int j = 0; j < values.size(); j++) {
			...
			if (Token.isTerminal(symbol) == false) { // resolve nonterminal
				try {
					...
					throw new ParserBuildException("FOLLOW set error: "
							+ ex.getMessage() + " <- " + symbol);
				}
			} else { // add terminal
	...
	/**
	 * Overridden to check for equality of key and value: Exception is thrown
	 * when so.
	 */
	@Override
	public Object put(Object key, Object value) {
		if (key.equals(value))
			throw new IllegalArgumentException(
					"Can not be FOLLOW of its own: key=" + key + ", value="
							+ value);
	...
	public static void main(String[] args) {
		...
		try {
			...
			FollowSets f = new FollowSets(syntax, nullAble, new FirstSets(
					syntax, nullAble, nt));
			String s = "T";
