 ...
 * <p>
 * The FIRST-set for a nonterminal is the collection of terminals that appear on
 * first position of the right side of all rules where that nonterminal appears
 * on the left side.
 * <p>
 * <b>Algorithm:</b><br>
 * Search all rules that contain the nonterminal on left side. Scan the left
 * side of every rule until a terminal or a non-nullable nonterminal appears.
 * The terminal goes to FIRST, the FIRST sets of all scanned nonterminals go to
 * FIRST. This implies that one must collect FIRST sets recursively, for every
 * scanned nonterminal.
 * 
...
class FirstSets extends UniqueAggregatingHashtable {
	/**
	 * Calculate Map of FIRST sets of all nonterminals contained in syntax. The
	 * FIRST sets will then be lists within this Map that provides the
	 * nonterminal as key.
	 * 
	 * @param syntax
	 *            the grammar to scan.
	 * @param nullAble
	 *            Map containing Boolean true or false for nullability of every
	 *            nonterminal.
	 * @param nonterminals
	 *            pre-built list of nonterminals
	 */
	...
	private void generateFirstSet(Syntax syntax, Nullable nullAble,
			String nonterminal, Map done) throws ParserBuildException {
		...
		for (int k = 0; k < syntax.size(); k++) {
			...
			if (nonterminal.equals(nonterm)) { // this rule derives the
												// nonterminal
				...
				if (rule.rightSize() <= 0) {
					...
					for (int i = 0; nullable && i < rule.rightSize(); i++) {
						...
						if (Token.isTerminal(symbol)) {
							...
							// If there is a nonterminal on first position, add
							// its FIRST set
							// FIRST set of this nonterminal, but without
							// null-word.
							try {
								generateFirstSet(syntax, nullAble, symbol, done); // enter
																					// recursion

								List list = (List) get(symbol); // get the
																// results
								...
								throw new ParserBuildException(ex.getMessage()
										+ " <- " + nonterm);
							}
						} // end if terminal
	...
	public Object put(Object key, Object value) {
		if (key.equals(value))
			throw new IllegalArgumentException(
					"Can not be FIRST of its own: key=" + key + ", value="
							+ value);
	...
	public static void main(String[] args) {
		...
		try {
			FirstSets f = new FirstSets(syntax, new Nullable(syntax, nonterm),
					nonterm);
			String s = "S";
