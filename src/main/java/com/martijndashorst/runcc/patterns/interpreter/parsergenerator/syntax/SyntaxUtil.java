...
public abstract class SyntaxUtil {
	...
	/**
	 * Appends the syntax array to receiving List. Does not check for
	 * uniqueness. Receiver list must not be null.
	 */
	public static List appendToSyntax(String[][] arrays, List receiver) {
		for (int i = 0; i < arrays.length; i++)
			receiver.add(appendToRule(arrays[i],
					new ArrayList(arrays[i].length)));
		return receiver;
	}
	...
	/**
	 * Build together some syntaxes to one List of Lists. Does not check for
	 * uniqueness.
	 */
	 ...
	 * (this is not really needed as Syntax checks for uniqueness on
	 * construction or when adding rules).
	 */
	public static final List catenizeRulesUnique(String[][][] arrays) {
		return catenizeRules(arrays, true);
	}

	private static final List catenizeRules(String[][][] arrays,
			boolean checkUnique) {
		int len = 0;
		for (int i = 0; i < arrays.length; i++)
			// count global length
