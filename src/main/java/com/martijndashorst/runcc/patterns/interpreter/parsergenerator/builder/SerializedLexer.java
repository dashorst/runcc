...
/**
 * Buffering Lexers. SerializedLexer will build the Lexer from scratch only the
 * first time. Every following call will load the serialized Lexer from
 * filesystem.
 * <p>
 * The time to build a lexer from scratch is equal to deserializing it in most
 * cases. So a standalone lexer can be built without this class. When needed for
 * a Parser, use the SerializedParser factory!
 * <p>
 * This factory will separate the passed syntax into parser and lexer syntax if
 * token and ignored symbol Lists are null. So take care to use "token" and
 * "ignored" rules within syntax to achieve the desired result!
 * <p>
...
public class SerializedLexer extends SerializedObject {
	...
	protected boolean PRODUCTION; // setting this to false in constructor will
									// prevent the Lexer from being serialized
	...
	/**
	 * Create a Lexer factory that caches built Lexers. @param production when
	 * false the Lexer will not be serialized.
	 */
	 ...
	 *            [][] or Syntax.
	 * @return deserialized Lexer, or one built from scratch that gets written
	 *         to filesystem.
	 */
	public Lexer get(Object syntaxInput) throws Exception {
	 ...
	 *         to filesystem.
	 */
	public Lexer get(Object syntaxInput, String baseName) throws Exception {
	 ...
	 *            File
	 * @param tokenSymbols
	 *            the token symbols when used by a prebuilt Parser
	 * @param ignoredSymbols
	 *            the ignored symbols when used by a prebuilt Parser
	 * @return deserialized Lexer, or one built from scratch that gets written
	 *         to filesystem.
	 */
	...
	/**
	 * Tries to read the lexer from a serialized file. One of the two arguments
	 * must be non-null.
	 * 
	 * @param syntaxInput
	 *            the lexer syntax input to retrieve a default name when it is a
	 *            File
	 * @param baseName
	 *            if baseName is "Xml", the file "XmlLexer.ser" will be read,
	 *            can be null
	 */
	...
	/**
	 * Builds a lexer from passed syntax and stores it to a File (when
	 * PRODUCTION is true, this is default).
	 * 
	 * @param syntaxInput
	 *            the lexer syntax input
	 * @param baseName
	 *            a file basename, if "Xml", the file "XmlLexer.ser" will be
	 *            written
	 */
	public Lexer buildAndStoreLexer(Object syntaxInput, String baseName,
			List tokenSymbols, List ignoredSymbols) throws Exception {
		...
		// else: assume that syntaxInput is a prebuilt lexer Syntax and a list
		// of ignored tokens
		...
		System.err.println("Lexer scratch construction took "
	 ...
	 * will not be null and can be used to retrieve the parser syntax, else null
	 * is returned, as the separation is not available in serialized Lexer.
	 */
	...
	/**
	 * Test main. Building serialized Lexer takes 330, building from scratch
	 * takes 130 millis.
	 */
	public static void main(String[] args) {
		try {
			TimeStopper ts = new TimeStopper();
			Lexer lexer = new SerializedLexer().get(
					StandardLexerRules.lexerSyntax, "SyntaxBuilder");
			System.err.println("Lexer was built in " + ts.getTimeMillis()
					+ " millis");
		} catch (Exception e) {
			e.printStackTrace();
		}
